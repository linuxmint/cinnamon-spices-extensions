  ...
  constructor() {
    this.shaderFactory = new ShaderFactory(Effect.getNick(), (shader) => {
      ...
      shader.connect('begin-animation', (shader, settings, forOpening, testMode) => {
        ...
        // code below.
        const gc =
          Clutter.Color.from_string(settings.getValue('hexagon-glow-color'))[1];
        const lc =
          Clutter.Color.from_string(settings.getValue('hexagon-line-color'))[1];

        // clang-format off
        shader.set_uniform_float(shader._uAdditiveBlending, 1, [settings.getValue('hexagon-additive-blending')]);
        shader.set_uniform_float(shader._uSeed,             2, [testMode ? 0 : Math.random(), testMode ? 0 : Math.random()]);
        shader.set_uniform_float(shader._uScale,            1, [settings.getValue('hexagon-scale')]);
        shader.set_uniform_float(shader._uLineWidth,        1, [settings.getValue('hexagon-line-width')]);

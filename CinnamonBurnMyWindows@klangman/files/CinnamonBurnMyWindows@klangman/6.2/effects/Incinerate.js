    ...
    this.shaderFactory = new ShaderFactory(Effect.getNick(), (shader) => {
      ...
      shader.connect(
        'begin-animation', (shader, settings, forOpening, testMode, actor) => {
          ...
          if (settings.getValue('incinerate-use-pointer')) {
          ...
          const c = Clutter.Color.from_string(settings.getValue('incinerate-color'))[1];

          // clang-format off
          shader.set_uniform_float(shader._uSeed,       2, seed);
          shader.set_uniform_float(shader._uColor,      3, [c.red / 255, c.green / 255, c.blue / 255]);
          shader.set_uniform_float(shader._uScale,      1, [settings.getValue('incinerate-scale')]);
          shader.set_uniform_float(shader._uTurbulence, 1, [settings.getValue('incinerate-turbulence')]);
          // clang-format on
        });

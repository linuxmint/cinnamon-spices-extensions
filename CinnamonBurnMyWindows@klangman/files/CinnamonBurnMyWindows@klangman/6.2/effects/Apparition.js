  ...
  constructor() {
    this.shaderFactory = new ShaderFactory(Effect.getNick(), (shader) => {
      ...
      shader.connect('begin-animation', (shader, settings, forOpening, testMode) => {
        ...
        shader.set_uniform_float(shader._uShake,      1, [settings.getValue('apparition-shake-intensity')]);
        shader.set_uniform_float(shader._uTwirl,      1, [settings.getValue('apparition-twirl-intensity')]);
        shader.set_uniform_float(shader._uSuction,    1, [settings.getValue('apparition-suction-intensity')]);
        shader.set_uniform_float(shader._uRandomness, 1, [settings.getValue('apparition-randomness')]);
        // clang-format on
      });
    });
  }

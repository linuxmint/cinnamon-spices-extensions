  ...
  constructor() {
    this.shaderFactory = new ShaderFactory(Effect.getNick(), (shader) => {
      ...
      shader.connect('begin-animation', (shader, settings, forOpening, testMode) => {
        const c = Clutter.Color.from_string(settings.getValue('portal-color'))[1];
        ...
        shader.set_uniform_float(shader._uDetails,       1, [settings.getValue('portal-details')]);
        shader.set_uniform_float(shader._uRotationSpeed, 1, [settings.getValue('portal-rotation-speed')]);
        shader.set_uniform_float(shader._uWhirling,         1, [settings.getValue('portal-whirling')]);
        // clang-format on
      });
    });
  }

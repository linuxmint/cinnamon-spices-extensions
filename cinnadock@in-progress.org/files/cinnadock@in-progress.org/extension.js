...
Dock.prototype = {
    _init : function() {
        ...
        this._styleChangedId = this.actor.connect('style-changed', Lang.bind(this, this._onStyleChanged));

        this._getPreferredWidthId = this._grid.connect('get-preferred-width', Lang.bind(this, this._getPreferredWidth));
        this._getPreferredHeightId = this._grid.connect('get-preferred-height', Lang.bind(this, this._getPreferredHeight));
        this._allocateId = this._grid.connect('allocate', Lang.bind(this, this._allocate));
        ...
        this._buttonPressEventId = this.actor.connect('button-press-event', Lang.bind(this, this._onButtonPress));

        this._settingsChangedId1 = this._settings.connect('changed::'+DOCK_POSITION_KEY, Lang.bind(this, function (){
                if (!this._settings)
                    return;
        ...
        this._settingsChangedId2 = this._settings.connect('changed::'+DOCK_SIZE_KEY, Lang.bind(this, function (){
                if (!this._settings)
                    return;
        ...
        this._settingsChangedId3 = this._settings.connect('changed::'+DOCK_HIDE_KEY, Lang.bind(this, function (){
                if (!this._settings)
                    return;
        ...
        this._settingsChangedId4 = this._settings.connect('changed::'+DOCK_EFFECTHIDE_KEY, Lang.bind(this, function () {
                if (!this._settings)
                    return;
        ...
        this._settingsChangedId5 = this._settings.connect('changed::'+DOCK_AUTOHIDE_ANIMATION_TIME_KEY, Lang.bind(this,function (){
                if (!this._settings)
                    return;
        ...
        this._leaveEventId = this.actor.connect('leave-event', Lang.bind(this, this._hideDock));
        this._enterEventId = this.actor.connect('enter-event', Lang.bind(this, this._showDock));
    ...
    destroy: function() {
        ...
        if (this._buttonPressEventId) {
                    this.actor.disconnect(this._buttonPressEventId);
        ...
        if (this._settingsChangedId3) {
                    this._settings.disconnect(this._settingsChangedId3);
                    this._settingsChangedId3 = 0;
        }

        if (this._settingsChangedId4) {
                    this._settings.disconnect(this._settingsChangedId4);
                    this._settingsChangedId4 = 0;
        }

        if (this._settingsChangedId5) {
                    this._settings.disconnect(this._settingsChangedId5);
                    this._settingsChangedId5 = 0;
        }
    ...
    _onStyleChanged: function() {
        ...
        if (success)
            this._spacing = len;
        [success, len] = themeNode.get_length('-cinnadock-grid-item-size', false);
        if (success)
            this._item_size = len;
    ...
    removeAll: function () {
        this._grid.get_children().forEach(Lang.bind(this, function (child) {
            //child.destroy();
            child.emit('destroy') //let dockicon handle destroying its actors
        }));
    },
...
DockMenu.prototype = {
    ...
    _init: function(dock, orientation) {
        ...
        this._onHideableChangeId = this.hideableSwitch.connect('activate', Lang.bind(this, this._onHideableChange));
    ...
    refreshPositionSubMenu: function() {
        ...
        for (let key in PositionMode) {
          ...
          positionItem.connId = positionItem.connect('activate', Lang.bind(this, function (actor, event) {
...
DockIconMenu.prototype = {
    ...
    _init: function(dockIcon, orientation) {
        ...
        //keep track of singnal connections so you can clean them up easily
        this._conns = [];
    },
    ...
    _redisplay: function() {
        ...
        if (!this._dockIcon.app.is_window_backed()) {
            ...
            item.connectId = item.connect('activate', Lang.bind(this, this._onNewWindow));
        ...
        for (let i = 0; i < windows.length; i++) {
            ...
            let item = new PopupMenu.PopupMenuItem(windows[i].title);
            this.addMenuItem(item);
            item._window = windows[i];
            item.connectId = item.connect('activate', Lang.bind(this, this._onActiveWindow));
            this._conns.push(item);
        }
        ...
        if (this._dockIcon._isPinned) {
            ...
            let item = new PopupMenu.PopupMenuItem(_('Remove from dock'));
            this.addMenuItem(item);
            item.connectId = item.connect('activate', Lang.bind(this, this._onRemove));
            this._conns.push(item);
        } else {
            let item = new PopupMenu.PopupMenuItem(_('Pin to dock'));
            this.addMenuItem(item);
            item.connectId = item.connect('activate', Lang.bind(this, this._onPinToDock));
            this._conns.push(item);
        }        
    },
    ...
    _onActiveWindow: function(actor, event) {
...
DockIcon.prototype = {
    _init : function(app, dock, isPinned) {
        ...
        this._actorDestroyId = this.actor.connect('destroy', Lang.bind(this, this._onDestroy));

        this._tracker = Cinnamon.WindowTracker.get_default();
        this._focusAppId = this._tracker.connect('notify::focus-app', Lang.bind(this, this._onStateChanged));

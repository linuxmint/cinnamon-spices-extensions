...
function disable() {
    if (signalSubscriptionId) {
        try {
            ...
            global.log('LayoutPopup: Error unsubscribing from D-Bus');
        }
...
function showLayoutPopup(text) {
    ...
    try {
        ...
        global.log('LayoutPopup: Using fallback font');
    }
    ...
    const containerStyle = 'background-color: ' + finalBgColor + '; ' +
    ...
    const labelStyle = 'font-family: "' + systemFont + '"; ' +
    ...
    targets.forEach(monitor => {
        ...
        let widthResult = container.get_preferred_width(-1);
        let naturalWidth = widthResult[1];
        let heightResult = container.get_preferred_height(-1);
        let naturalHeight = heightResult[1];
...
// Функции hideAllOSDs, destroyAllOSDs остаются без изменений

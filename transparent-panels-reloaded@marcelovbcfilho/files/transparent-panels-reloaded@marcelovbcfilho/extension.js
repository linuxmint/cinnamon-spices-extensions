...
MyExtension.prototype = {
    _init: function (meta) {
        ...
        this.settings.bind("transparency-amount", "transparency_amount", this.on_settings_changed);
        this.settings.bind("transparency-always", "transparency_always", this.on_settings_changed);
        this.settings.bind("use-custom-panel-color", "use_custom_panel_color", this.on_settings_changed);
        this.settings.bind("custom-panel-color", "custom_panel_color", this.on_settings_changed);
        ...
        this._classname = this.use_custom_panel_color ? TRANSPARENT_THEME : DEFAULT_THEME;
    ...
    on_state_change: function (monitor) {
        this._filter.for_each_panel(panel => {
            let transparentize = this.policy.is_transparent(panel) || this.transparency_always;
    ...
    make_transparent: function (panel, transparent) {
        ...
        if (transparent) {
            this._set_background_opacity(panel, 255 * (this.transparency_amount / 100));
        } else {
    ...
    _set_background_opacity: function (panel, alpha) {
        ...
        console.error(`rgba(${color.red}, ${color.green}, ${color.blue}, ${color.alpha})`)
    ...
    _update_filter: function () {
        if (this.enable_position_top) {
            this._filter.add(Panel.PanelLoc.top);
        } else {
            this._filter.remove(Panel.PanelLoc.top);
        }

        if (this.enable_position_right) {
            this._filter.add(Panel.PanelLoc.right);
        } else {
            this._filter.remove(Panel.PanelLoc.right);
        }

        if (this.enable_position_bottom) {
            this._filter.add(Panel.PanelLoc.bottom);
        } else {
            this._filter.remove(Panel.PanelLoc.bottom);
        }

        if (this.enable_position_left) {
    ...
    on_settings_changed: function () {
        this._classname = this.theme_color ? TRANSPARENT_THEME : DEFAULT_THEME;
